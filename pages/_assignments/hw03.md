...
For more advanced queries, you can also use the regular SQL Alchemy documentation (<a href="https://docs.sqlalchemy.org/en/14/orm/tutorial.html#common-filter-operators" target="_blank">SQL Alchemy documentation here</a>). Some samples are pasted below:
...
# 1. get all posts:
posts = Post.query.order_by(Post.user_id).all()
...
posts = Post.query.filter(Post.user_id.in_([3, 4, 5]))
...
# 6. Join (three tables) to select particular columns and the comment count:
