...
Yyou will also be implementing some event handlers to interact with the REST API that you made -- in order to like, bookmark, and comment on posts; and to follow new users. And finally, you'll continue working to ensure that your interface is accessible.

> ### A few high-level suggestions / notes:
...
## 2. Set Up
In order to make this assignment a little simpler, we have some suggested adjustments that we would like you to make, which are included in the HW04 starter files. Specifically:
...
<a class="nu-button" href="/winter2022/course-files/homework/hw03.zip">hw04.zip<i class="fas fa-download" aria-hidden="true"></i></a>
...
{:.compact}
1. Copy `api_structure.py`, `post.py`, and `utils.py` from `hw04/models` into `photo-app/models`.
...
Navigate to `photo-app` on your command line, and run flask (`flask run`). You should see the following screens when navigating to <a href="http://localhost:5000" target="_blank">http://localhost:5000</a> <a href="http://localhost:5000/api" target="_blank">http://localhost:5000/api</a> respectively:
...
### 4. Tips
While we've provided some starter HTML and CSS (e.g., `templates/starter-client.html` and `static/css/starter-client.css`), you are encouraged to use the HTML and CSS that you made in HW1 & HW2. If the starter code doesn't help, feel free to do it your way.

## 3. Your Tasks
Please complete the following tasks. Feel free to use the <a href="http://localhost:5000/api" target="_blank">API tester</a> that we made to help you. 
